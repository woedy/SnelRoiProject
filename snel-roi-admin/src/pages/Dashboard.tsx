...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [users, accounts, transactions, unreadCount] = await Promise.all([
          userService.getAll(),
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{stats.unreadSupport}</div>
            <p className="text-xs text-muted-foreground">Unread customer messages</p>
          </CardContent>
        </Card>
      </div>
    </div>
